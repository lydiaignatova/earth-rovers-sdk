...
tmux send-keys -t frodobot:0.2 'conda activate frodobot' C-m
...
tmux send-keys -t frodobot:0.3 'conda activate frodobot' C-m
tmux send-keys -t frodobot:0.3 'cd ~/repos/earth-rovers-sdk/utils' C-m
tmux send-keys -t frodobot:0.3 "curl --location 'http://localhost:8000/data'" C-m
